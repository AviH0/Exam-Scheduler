...
def run_solver(solver_type: str, dl: CSVdataloader, evaluator: Evaluator) -> Tuple[Solver, State]:
    ...
    if solver_type == GENETIC_SOL:
        ...
        solver = SAsolver(dl, evaluator, YearSemester.SEM_A)
        sol = solver.solve()
...
def main():
    dl = CSVdataloader(major_data_dir="data/data.csv", startA=date(2021, 7, 1), endA=date(2021, 8, 1),
                       startB=date(2021, 8, 1), endB=date(2021,8,20), not_allowed=[],
